...
public class AppSharedPreferences {
    ...
    public AppSharedPreferences(Context appContext) {
        PACKAGE_NAME="com.example.gresa.FIEK-FaceRecognizer";
        preferences = appContext.getSharedPreferences(PACKAGE_NAME,Context.MODE_PRIVATE);
    ...
    public ArrayList<String> getListString(String key) {
        ...
        if(!value.equals(""))
        return new ArrayList<String>(Arrays.asList(TextUtils.split(value, "‚‗‚")));
        else
            return null;
    }
        ...
        if(objStrings!=null) {
            for (String jObjString : objStrings) {
                ...
                if (decodedByte!=null) {
                    ...
                    Mat mat=new Mat(decodedByte.getWidth(),decodedByte.getHeight(),1);
                    mat=imdecode(new Mat(data),0);

                    Log.d("MatDecode","rows:"+mat.rows()+"/"+decodedByte.getWidth()+" cols:"+mat.cols()+"/"+decodedByte.getHeight());
                    if(mat!=null)
                    objects.add(mat);
                }
                else{
    ...
    public Mat getMat(String key){
        String matEncoded = getString(key);

            byte[] data = Base64.decode(matEncoded, Base64.DEFAULT);
            Bitmap decodedByte = BitmapFactory.decodeByteArray(data, 0, data.length);
            OpenCVFrameConverter.ToMat matConverter=new OpenCVFrameConverter.ToMat();
            opencv_core.IplImage image= opencv_core.IplImage.create(decodedByte.getWidth(),decodedByte.getHeight(),IPL_DEPTH_32S,1);
            Mat mat=new Mat();
            Frame frame= matConverter.convert(image);
            mat=matConverter.convertToMat(frame);
...
//    }
    ...
    public void putListMat(String key, ArrayList<Bitmap> objArray){
        checkForNullKey(key);
        ArrayList<String> objStrings = new ArrayList<String>();

        for (Bitmap bmp : objArray) {
            ...
            if(data.length!=0) {
                ...
                Log.d("SharedPrefs","Can't add null mat");
        }
        putListString(key, objStrings);
    }
    /*
    public void putMat(String key, Bitmap objBitmap){
        checkForNullKey(key);

            OpenCVFrameConverter.ToMat matConverter=new OpenCVFrameConverter.ToMat();
            opencv_core.IplImage image= opencv_core.IplImage.create(objBitmap.getWidth(),objBitmap.getHeight(),IPL_DEPTH_32S,1);
            Mat mat;
            Frame frame= matConverter.convert(image);
            mat=matConverter.convertToMat(frame);
            ...
            mat.data().get(data);
            String matEncoded = new String(Base64.encode(data, Base64.DEFAULT));


        putString(key,matEncoded);
    }*/
